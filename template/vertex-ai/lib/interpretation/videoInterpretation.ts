...
async function generateCoverageMap(
  ...
  const buildPrompt = (retryHint?: string) =>
    [
      ...
      '',
      'Format meaning:',
      '- After SUMMARY:, write only one sentence with the core thesis.',
      isDetailed
      ...
      isDetailed ? '- Detailed mode: prioritize data, cases, evidence, and reasoning-chain details.' : '',
...
async function generateFullArticle(
  ...
  const buildPrompt = (retry: boolean) =>
    [
      ...
      '2) Stay faithful to transcript facts; do not fabricate details.',
      ...
      isDetailed
        ...
        : '',
      retry ? '9) Retry: previous output was incomplete. Return a complete article.' : '',
      '',
      'Mandatory key points to cover:',
